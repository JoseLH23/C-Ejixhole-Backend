...
from app.core.rate_limiter import limitar_publico, limitar_reservacion_publica
...
@router.get("/servicios", response_model=list[ServicioPublicoOut], dependencies=[Depends(limitar_publico)])
def listar_servicios_informativos(db: Session = Depends(get_db)):
    return PublicoService(db).listar_servicios_informativos()


@router.get("/unidades-hospedaje", response_model=list[UnidadHospedajePublicoOut], dependencies=[Depends(limitar_publico)])
def listar_unidades_hospedaje(db: Session = Depends(get_db)):
    return PublicoService(db).listar_unidades_hospedaje()


@router.get("/fechas-bloqueadas", response_model=list[FechaBloqueadaPublicaOut], dependencies=[Depends(limitar_publico)])
...
@router.get("/disponibilidad-calendario", response_model=list[PeriodoNoDisponibleOut], dependencies=[Depends(limitar_publico)])
...
@router.get("/disponibilidad", response_model=DisponibilidadOut, dependencies=[Depends(limitar_publico)])
...
@router.get("/cotizar", response_model=CotizacionOut, dependencies=[Depends(limitar_publico)])
...
@router.get("/form-challenge", response_model=FormChallengeOut, dependencies=[Depends(limitar_publico)])
def obtener_desafio_formulario(response: Response, db: Session = Depends(get_db)):
    response.headers["Cache-Control"] = "no-store"
    response.headers["Pragma"] = "no-cache"
    return FormularioGuardService(db).crear_desafio()
...
def crear_solicitud_reservacion(data: ReservacionPublicaCreate, request: Request, db: Session = Depends(get_db)):
    ...
    def crear():
        limitar_reservacion_publica(request, data.email, data.telefono)
        guardia = FormularioGuardService(db)
        intento = guardia.reservar(request, data)
        try:
