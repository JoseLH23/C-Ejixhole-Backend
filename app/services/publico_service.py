...
class PublicoService:
    ...
    def listar_periodos_no_disponibles(self, unidad_hospedaje_id: int, desde: date, hasta: date) -> list[dict]:
        """Devuelve ocupación y cierres sin exponer clientes, títulos o notas.

        Las reservaciones ocupan noches en [llegada, salida), por lo que el
        día de salida no se marca ocupado. Los bloqueos administrativos sí
        son inclusivos en ambos extremos.
        """
        self._obtener_unidad_activa(unidad_hospedaje_id)
        periodos: list[dict] = []
