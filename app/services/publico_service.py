...
class PublicoService:
    ...
    def listar_servicios_informativos(self) -> list[Servicio]:
        return self.db.query(Servicio).filter(
            Servicio.reservable.is_(False),
            Servicio.activo.is_(True),
        ).order_by(Servicio.nombre).all()

    def listar_unidades_hospedaje(self) -> list[UnidadHospedaje]:
        return self.db.query(UnidadHospedaje).filter(
            UnidadHospedaje.activa.is_(True)
        ).order_by(UnidadHospedaje.nombre).all()

    def _obtener_unidad_activa(self, unidad_hospedaje_id: int) -> UnidadHospedaje:
        unidad = self.db.query(UnidadHospedaje).filter(
            UnidadHospedaje.id == unidad_hospedaje_id
        ).first()
    ...
    def hay_disponibilidad(self, unidad_hospedaje_id: int, fecha_llegada: date, fecha_salida: date) -> bool:
        ...
        return not self.reservacion_service.repo.existe_traslape_unidad_hospedaje(
            unidad_hospedaje_id,
            fecha_llegada,
            fecha_salida,
        )
    ...
    def listar_periodos_no_disponibles(self, unidad_hospedaje_id: int, desde: date, hasta: date) -> list[dict]:
        ...
        for bloqueo in bloqueos:
            periodos.append({
                "fecha_inicio": max(bloqueo.fecha_inicio, desde),
                "fecha_fin": min(bloqueo.fecha_fin, hasta),
                "motivo": "bloqueado",
            })
    ...
    def _buscar_o_crear_cliente(self, nombre_completo: str, email: str, telefono: str) -> Cliente:
        ...
        coincidencia_segura = next(
            (cliente for cliente in coincidencias if cliente.telefono == telefono and cliente.email == email),
            None,
        )
        if coincidencia_segura is not None:
            return coincidencia_segura
        return self.cliente_repo.crear(
            Cliente(nombre=nombre_completo, email=email, telefono=telefono)
        )
    ...
    def _resolver_servicio_id(self, tipo_reservacion: str, unidad_hospedaje_id: int | None) -> int:
        if tipo_reservacion == "entrada":
            ...
            if not servicio:
                raise HTTPException(
                    status_code=500,
                    detail=f"No hay un servicio '{nombre}' configurado en el catálogo.",
                )
            return servicio.id
